    ...
    [UIView animateKeyframesWithDuration:transitionDuration
                              ...
                              animations:^{
                                  [UIView addKeyframeWithRelativeStartTime:0.0 relativeDuration:0.6 animations:^{
                                      if(mode == KASlideShowSlideModeBackward){
                                          self->_topImageView.transform = CGAffineTransformMakeTranslation( self->_topImageView.frame.size.width * 0.9, 0);
                                          self->_bottomImageView.transform = CGAffineTransformMakeTranslation(- self->_bottomImageView.frame.size.width * 0.1, 0);
                                      }else if(mode == KASlideShowSlideModeForward){
                                          self->_topImageView.transform = CGAffineTransformMakeTranslation(- self->_topImageView.frame.size.width * 0.9, 0);
                                          self->_bottomImageView.transform = CGAffineTransformMakeTranslation(self->_bottomImageView.frame.size.width * 0.1, 0);
                                      }
